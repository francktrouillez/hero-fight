class Mirror extends ComplexObject {
  ...
  constructor(gl, obj_content, main_camera) {
    ...
    this.mirror_camera = new SquaredViewCamera({
      eye: {
        x: 0.0, y: 0.0, z: 0.0
      },
      center: {
  ...
  init_buffers() {
    ...
    this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, this.resolution, this.resolution, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, null);
  ...
  update_mirror_refraction() {
    ...
    const theta_2 = Math.abs(Math.asin(clamp(Math.sin(theta_1)/this.refraction_indice, -1, 1)));
    
    const old_rel_z = get_perpendicular_vector(vect_eye_to_mirror, normal);
    const old_rel_y = normal;
    const old_rel_x = get_perpendicular_vector(old_rel_y, old_rel_z);

    const distance_eye_to_mirror = norm(vect_eye_to_mirror);

    this.mirror_camera.set_center({
      x: mirror_eye.x + distance_eye_to_mirror * (- Math.cos(theta_2) * old_rel_y.x + Math.sin(theta_2) * old_rel_x.x), 
      y: mirror_eye.y + distance_eye_to_mirror * (- Math.cos(theta_2) * old_rel_y.y + Math.sin(theta_2) * old_rel_x.y), 
      z: mirror_eye.z + distance_eye_to_mirror * (- Math.cos(theta_2) * old_rel_y.z + Math.sin(theta_2) * old_rel_x.z), 
    })
    
    const r = this.radius_object;
    const alpha = this.mirror_camera.info.fov * Math.PI/180 / 2;

    const dist_1 = r * Math.sin(Math.PI/2 + theta_2 - alpha)/Math.sin(alpha)
    const dist_2 = r * Math.sin(Math.PI/2 - theta_2 - alpha)/Math.sin(alpha)

    const dist = Math.min(dist_1, dist_2)

    const near = Math.sqrt(dist * dist + r * r - 2 * dist * r * Math.cos(theta_2))  
    ...
    const dist_eye_to_image_plan = dist - r * Math.cos(theta_2);
    const center_image_plan = {
      x: this.mirror_camera.info.eye.x + optical_axis_mirror.x * dist_eye_to_image_plan,
      y: this.mirror_camera.info.eye.y + optical_axis_mirror.y * dist_eye_to_image_plan,
      z: this.mirror_camera.info.eye.z + optical_axis_mirror.z * dist_eye_to_image_plan,
