class SlimeRender extends RenderObject {
  ...
  constructor(gl, program, camera, lights_list) {
    ...
    for (let i = 0; i <= 20; i++) {
      obj_animation_map["idle"].push(obj_files["./src/view/assets/models/Slime/idle/" + i + ".obj"])
    }
    for (let i = 0; i <= 15; i++) {
      obj_animation_map["attack"].push(obj_files["./src/view/assets/models/Slime/attack/" + i + ".obj"])
    }

    var uniform_map = {}

    super(
      new AnimatedObject(gl, obj_files["./src/view/assets/models/Slime/Slime.obj"], obj_animation_map),
      program,
      camera,
      uniform_map
    );
